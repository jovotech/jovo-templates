...
app.setHandler({
	...
	StartState: {
		StartGameIntent() {
			...
			const roundAnswers: string[] = populateRoundAnswers(
				gameQuestions,
				0,
				correctAnswerIndex,
				translatedQuestions
...
function populateRoundAnswers(
	...
	const answersCopy: string[] = translatedQuestions[gameQuestionIndexes[correctAnswerIndex]][
...
function handleUserGuess(instance: Jovo, userGaveUp: boolean) {
	...
	if (instance.getSessionAttribute('currentQuestionIndex') === GAME_LENGTH - 1) {
		...
		instance.$reprompt.t('TELL_QUESTION_MESSAGE', {
