			...
			this.setSessionAttributes({
				...
				correctAnswerText:
					translatedQuestions[gameQuestions[currentQuestionIndex]][
						Object.keys(translatedQuestions[gameQuestions[currentQuestionIndex]])[0]
					][0],
			});
...
function populateRoundAnswers(gameQuestionIndexes, correctAnswerIndex, correctAnswerTargetLocation, translatedQuestions) {
	const answers = [];
	const answersCopy = translatedQuestions[gameQuestionIndexes[correctAnswerIndex]][
...
function isAnswerSlotValid(answer) {
	if (answer) {
		return (
			!isNaN(parseInt(answer.value, 10)) &&
			parseInt(answer.value, 10) < ANSWER_COUNT + 1 &&
			parseInt(answer.value, 10) > 0
		);
	} else {
		return false;
	}
...
function handleUserGuess(userGaveUp) {
	...
	if (this.getSessionAttribute('currentQuestionIndex') === GAME_LENGTH - 1) {
		...
		this.$reprompt.t('TELL_QUESTION_MESSAGE', {
			questionNumber: questionIndexForSpeech.toString(),
