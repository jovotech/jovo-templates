...
app.setHandler({
  LAUNCH() {
    ...
    if (this.$alexaSkill && this.$alexaSkill.$gameEngine) {
      ...
      if (this.$alexaSkill.gadgetController()) {
      ...
      this.$alexaSkill.$gameEngine.respond(this.$speech.build());
    }
  },
  ...
  ON_GAME_ENGINE_INPUT_HANDLER_EVENT() {
    ...
    if (this.$alexaSkill) {
      ...
      if (eventName === 'timeoutEvent') {
        ...
        console.log(`Matching request ID? ${this.$user.$data.currentInputHandlerId === originatingRequestId}`);
        ...
        console.log(`Known buttons: ${JSON.stringify(knownButtons, null, 4)}`);
        ...
        if (_.includes(knownButtons, buttonId)) {
          ...
          this.$alexaSkill.$gadgetController!.setLight([buttonId], 0, '', [
            {
              repeat: 20,
              targetLights: ['1'],
              sequence: [
        ...
        this.$alexaSkill.$gadgetController!.respond(this.$speech.build());
      }
    }
  },
